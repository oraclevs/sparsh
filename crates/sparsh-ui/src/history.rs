use reedline::{
    FileBackedHistory, History, HistoryItem, HistoryItemId, HistorySessionId, Result as ReedlineResult,
...
impl SharedHistory {
    pub(crate) fn new(history: SparshHistory) -> Self {
        Self { inner: Arc::new(Mutex::new(history)) }
    }
}

impl sparsh_core::HistoryAccess for SharedHistory {
    fn list(&self, limit: Option<usize>) -> Result<Vec<String>, String> {
        let history = self.inner.lock().map_err(|_| "history lock poisoned".to_string())?;
...
impl History for SharedHistory {
    fn save(&mut self, item: HistoryItem) -> ReedlineResult<HistoryItem> { self.inner.lock().unwrap().save(item) }
    fn load(&self, id: HistoryItemId) -> ReedlineResult<HistoryItem> { self.inner.lock().unwrap().load(id) }
    fn count(&self, query: SearchQuery) -> ReedlineResult<i64> { self.inner.lock().unwrap().count(query) }
    fn search(&self, query: SearchQuery) -> ReedlineResult<Vec<HistoryItem>> { self.inner.lock().unwrap().search(query) }
    fn update(&mut self, id: HistoryItemId, updater: &dyn Fn(HistoryItem) -> HistoryItem) -> ReedlineResult<()> { self.inner.lock().unwrap().update(id, updater) }
    fn clear(&mut self) -> ReedlineResult<()> { self.inner.lock().unwrap().clear() }
    fn delete(&mut self, id: HistoryItemId) -> ReedlineResult<()> { self.inner.lock().unwrap().delete(id) }
    fn sync(&mut self) -> std::io::Result<()> { self.inner.lock().unwrap().sync() }
    fn session(&self) -> Option<HistorySessionId> { self.inner.lock().unwrap().session() }
}
